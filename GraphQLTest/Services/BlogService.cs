...
namespace GraphQLTest.Services
{
    public class BlogService
    {
        ...
        private readonly IMongoCollection<Author> _authorsDatabaseService;
        private readonly IMongoCollection<Post> _postsDatabaseService;
        private readonly IMongoCollection<SocialNetwork> _snsDatabaseService;
        ...
        public BlogService()
        {
            ...
            _authorsDatabaseService = database.GetCollection<Author>("userDB");
            _postsDatabaseService = database.GetCollection<Post>("userDB");
            _snsDatabaseService = database.GetCollection<SocialNetwork>("userDB");

            authors = _authorsDatabaseService.Find(it => true).ToList();
            posts = _postsDatabaseService.Find(it => true).ToList();
            sns = _snsDatabaseService.Find(it => true).ToList();
        }
        //mock data
        //public BlogService()
        //{
